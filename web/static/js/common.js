	 ...
	 */	
	 $.fn.search = function(onsearch, onconf){		 
	 	return this.each(function(){	
		 	...
		 	var search = function(){
		 		var e = $("#search").attr("url"),
		        t = $(".search").find("input").serialize();		 		
		    	t = t.replace(/(&|^)(\w*?\d*?\-*?_*?)*?=?((?=&)|(?=$))/g, ""), t = t.replace(/(^&)|(\+)/g, ""), e += e.indexOf("?") > 0 ? "&" + t : "?" + t, window.location.href = e
		 	}
		 	var config = function() {
		 		//$("#search_advance").css("display",'block');
		 		var val = $input.val();
...
$(function() {	
    if ("standalone" in window.navigator && window.navigator.standalone) {    	
        ...
        document.addEventListener("click", function(a) {
            for (e = a.target;
                "A" !== e.nodeName && "HTML" !== e.nodeName;) e = e.parentNode;
            "href" in e && -1 !== e.href.indexOf("http") && (-1 !== e.href.indexOf(document.location.host) || t) && (a.preventDefault(), document.location.href = e.href)
        }, !1)
    }
    $.alertMessager = function(e, t, a) {
        if ($.bootstrapGrowl) $.bootstrapGrowl(e, {
            type: t,
            delay: 2e3,
            align: "center",
            width: "auto"
            ...
            t = t ? t : "danger";            
            var s = '<div style="padding:15px 30px 15px 20px;width:300px;height:auto;margin:0 auto;max-width: 80%;top:52px;left:0;right:0;z-index:99999;"class="messager navbar-fixed-top border-none alert alert-' + t + '"><button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">×</span></button>' + e + "</div>";
            $(".messager").remove(), $("body").prepend(s), setTimeout(function() {
                $(".messager").remove()
            }, a ? a : 2e3)
        }
    },    
    $(".table .check-all").click(function() {
    	if (this.checked)
    		$(".checkbox").addClass('checked',this.checked);
    	else
    ...
    $("body").on("click", ".ajax-get", function() {
        var e, t = this;
        return $(this).hasClass("confirm") && !confirm("确认要执行该操作吗?") ? !1 : (((e = $(this).attr("href")) || (e = $(this).attr("url"))) && ($(this).addClass("disabled").attr("autocomplete", "off").prop("disabled", !0), $.get(e).success(function(e) {
            1 == e.status ? (message = e.url ? e.info + " 页面即将自动跳转~" : e.info, $.alertMessager(message, "success"), setTimeout(function() {
                $(t).removeClass("disabled").prop("disabled", !1), e.url ? location.href = e.url : location.reload()
            }, 2e3)) : (1 == e.login ? $("#login-modal").modal() : $.alertMessager(e.info, "danger"), setTimeout(function() {
                $(t).removeClass("disabled").prop("disabled", !1)
            }, 5e3))
        })), !1)
    }), $("body").on("click", ".ajax-post", function() {
        var e, t, a, s = $(this).attr("target-form"),
            i = this,
            n = !1;
        if ("submit" == $(this).attr("type") || (e = $(this).attr("href")) || (e = $(this).attr("url"))) {
            if (a = $("." + s), "true" === $(this).attr("hide-data")) a = $(".hide-data"), t = a.serialize();
            else {
                if (void 0 == a.get(0)) return !1;
                if ("FORM" == a.get(0).nodeName) {
                    if ($(this).hasClass("confirm") && !confirm("确认要执行该操作吗?")) return !1;
                    e = void 0 !== $(this).attr("url") ? $(this).attr("url") : a.get(0).action, t = a.serialize()
                } else if ("INPUT" == a.get(0).nodeName || "SELECT" == a.get(0).nodeName || "TEXTAREA" == a.get(0).nodeName) {
                    if (a.each(function(e, t) {
                        "checkbox" == t.type && 1 == t.checked && (n = !0)
                    }), n && $(this).hasClass("confirm") && !confirm("确认要执行该操作吗?")) return !1;
                    t = a.serialize()
                } else {
                    if ($(this).hasClass("confirm") && !confirm("确认要执行该操作吗?")) return !1;
                    t = a.find("input,select,textarea").serialize()
                }
            }
            $(i).addClass("disabled").attr("autocomplete", "off").prop("disabled", !0), $.post(e, t).success(function(e) {
                1 == e.status ? (message = e.url ? e.info + " 页面即将自动跳转~" : e.info, $.alertMessager(message, "success"), setTimeout(function() {
                    e.url ? location.href = e.url : location.reload()
                }, 2e3)) : ($.alertMessager(e.info, "danger"), setTimeout(function() {
                    $(i).removeClass("disabled").prop("disabled", !1)
                }, 2e3), $(".reload-verify").length > 0 && $(".reload-verify").click())
            })
        }
        return !1
    }),   
    ...
    function(e) {
        ...
        e.fn.Huploadify = function(t) {
            var a = '<div id="${fileID}" class="uploadify-queue-item"><div class="uploadify-progress"><div class="uploadify-progress-bar"></div></div><span class="up_filename">${fileName}</span><span class="uploadbtn">上传</span><span class="delfilebtn">删除</span></div>',
                s = {
                    fileTypeExts: "*.*",
                    uploader: "",
                    auto: !0,
                ...
                n = function(e, t) {
                    return e = e > 1048576 && !t ? (Math.round(100 * e / 1048576) / 100).toString() + "MB" : (Math.round(100 * e / 1024) / 100).toString() + "KB"
                }, o = function(e, t) {
                    for (var a = 0; a < t.length; a++)
                        if (t[a].index == e) return t[a];
                    return !1
                }, r = function(e) {
                    for (var t = [], a = e.split(";"), s = 0, i = a.length; i > s; s++) t.push(a[s].split(".").pop());
                    return t
                }, l = {
                    ...
                    for (var t = r(e), a = [], s = 0, i = t.length; i > s; s++) {
                        var n = c(t[s]);
                        n && a.push(n)
                    }
                    return a.join(",")
                }, h = function(e, t, a, s) {
                    t.open(i.method, e, !0), t.setRequestHeader("X-Requested-With", "XMLHttpRequest");
                    var n = new FormData;
                    if (n.append(i.fileObjName, a), s)
                        for (key in s) n.append(key, s[key]);
                    t.send(n)
                }, d = null;
            this.each(function() {
                var t = e(this),
                    a = e(".uploadify").length + 1,
                    s = '<input id="select_btn_' + a + '" class="selectbtn" style="display:none;" type="file" name="fileselect[]"';
                s += i.multi ? " multiple" : "", s += ' accept="', s += f(i.fileTypeExts), s += '"/>', s += '<a id="file_upload_' + a + '-button" href="javascript:void(0)" class="uploadify-button btn btn-primary">', s += i.buttonText, s += "</a>";
                var o = '<div id="file_upload_' + a + '-queue" class="uploadify-queue"></div>';
                t.append(s + o), d = {
                    uploadAllowed: !0,
                    fileInput: t.find(".selectbtn"),
                    uploadFileList: t.find(".uploadify-queue"),
                    container: t,
                    url: i.uploader,
                    fileFilter: [],
                    uploadOver: !1,
                    filter: function(t) {
                        ...
                        if (s.length > 0)
                            for (var o = 0, l = t.length; l > o; o++) {
                                var c = t[o];
                                parseInt(n(c.size, !0)) > i.fileSizeLimit ? alert("文件" + c.name + "大小超出限制！") : e.inArray(c.name.split(".").pop().toLowerCase(), s) >= 0 || e.inArray("*", s) >= 0 ? a.push(c) : alert("文件" + c.name + "类型不允许！")
                            }
                        return a
                    },
                    funSelect: function(s) {
                        for (var o = 0, r = s.length; r > o; o++) {
                            var l = s[o],
                                c = e(i.itemTemplate.replace(/\${fileID}/g, "fileupload_" + a + "_" + l.index).replace(/\${fileName}/g, l.name).replace(/\${fileSize}/g, n(l.size)).replace(/\${instanceID}/g, t.attr("id")));
                            i.auto && c.find(".uploadbtn").remove();
                            var f = 0,
                                h = "0KB",
                                p = "0%";
                            if (i.breakPoints) {
                                var u = this.funGetUploadedSize(l);
                                f = u / l.size * 100 + "%", h = n(u), p = (u / l.size * 100).toFixed(2) + "%", c.find(".uploadify-progress-bar").css("width", f)
                            }
                            if (this.uploadFileList.append(c), i.showUploadedSize) {
                                var m = '<span class="progressnum"><span class="uploadedsize">' + h + '</span>/<span class="totalsize">${fileSize}</span></span>'.replace(/\${fileSize}/g, n(l.size));
                                c.find(".uploadify-progress").after(m)
                            }
                            if (i.showUploadedPercent) {
                                var g = '<span class="up_percent">' + p + "</span>";
                                c.find(".uploadify-progress").after(g)
                            }
                            i.onSelect && i.onSelect(s), i.auto ? this.funUploadFile(l) : c.find(".uploadbtn").on("click", function(e) {
                                return function() {
                                    d.funUploadFile(e)
                                }
                            }(l)), c.find(".delfilebtn").on("click", function(e) {
                                return function() {
                                    d.funDeleteFile(e.index)
                                }
                            }(l))
                        }
                    },
                    onProgress: function(e, s, o) {
                        var r = t.find("#fileupload_" + a + "_" + e.index + " .uploadify-progress"),
                            l = s,
                            c = r.attr("lastLoaded") || 0;
                        s -= parseInt(c);
                        var f = r.children(".uploadify-progress-bar"),
                            h = i.breakPoints ? parseFloat(f.get(0).style.width || 0) : 0,
                            d = (s / o * 100 + h).toFixed(2),
                            p = d > 100 ? "99.99%" : d + "%";
                        i.showUploadedSize && (r.nextAll(".progressnum .uploadedsize").text(n(s)), r.nextAll(".progressnum .totalsize").text(n(o))), i.showUploadedPercent && r.nextAll(".up_percent").text(p), f.css("width", p), l < i.fileSplitSize ? r.attr("lastLoaded", l) : r.removeAttr("lastLoaded")
                    },
                    funGetProgressWidth: function(e) {
                        var s = t.find("#fileupload_" + a + "_" + e + " .uploadify-progress-bar");
                        return s.get(0).style.width || ""
                    },
                    funGetUploadedSize: function(e) {
                        return i.getUploadedSize ? i.getUploadedSize(e) : i.saveInfoLocal ? parseInt(localStorage.getItem(e.name)) || 0 : void 0
                    },
                    funSaveUploadedSize: function(e, t) {
                        i.saveUploadedSize ? i.saveUploadedSize(e, t) : i.saveInfoLocal && localStorage.setItem(e.name, t)
                    },
                    funGetFiles: function(e) {
                        var t = e.target.files;
                        t = this.filter(t);
                        for (var a = 0, s = t.length; s > a; a++) this.fileFilter.push(t[a]);
                        return this.funDealFiles(t), this
                    },
                    funDealFiles: function(e) {
                        for (var s = t.find(".uploadify-queue .uploadify-queue-item").length, i = 0, n = e.length; n > i; i++) e[i].index = ++s, e[i].id = "fileupload_" + a + "_" + e[i].index;
                        return this.funSelect(e), this
                    },
                    funDeleteFile: function(e) {
                        for (var s = 0, n = this.fileFilter.length; n > s; s++) {
                            var o = this.fileFilter[s];
                            if (o.index == e) {
                                i.breakPoints && (this.uploadAllowed = !1), this.fileFilter.splice(s, 1), t.find("#fileupload_" + a + "_" + e).fadeOut(), d.fileInput.val(""), i.onCancel && i.onCancel(o);
                                break
                            }
                        }
                        return this
                    ...
                    funUploadFile: function(e) {
                        var s = !1,
                            n = e,
                            o = t.find("#fileupload_" + a + "_" + e.index),
                            r = function() {
                                d.uploadOver && (o.find(".uploadify-progress-bar").css("width", "100%"), i.showUploadedSize && o.find(".uploadedsize").text(o.find(".totalsize").text()), i.showUploadedPercent && o.find(".up_percent").text("100%"))
                            };
                        try {
                        ...
                        if (i.breakPoints) {
                            var c = e.name,
                                ...
                                p = e.index,
                                u = e.size,
                                m = parseInt(this.funGetUploadedSize(n));
                            e = n.slice(m, m + i.fileSplitSize), e.name = c, e.id = f, e.index = p
                        }
                        s.upload && m !== !1 && (s.upload.addEventListener("progress", function(t) {
                            d.onProgress(e, t.loaded, n.size)
                        }, !1), s.onreadystatechange = function(o) {
                            if (4 == s.readyState) {
                                if (d.uploadOver = !0, 200 == s.status) {
                                    var l = JSON.parse(s.responseText);
                                    l.success && (i.breakPoints ? (m += i.fileSplitSize, d.funSaveUploadedSize(n, m), u > m ? (d.uploadOver = !1, d.uploadAllowed && (e = n.slice(m, m + i.fileSplitSize), e.name = c, e.id = f, e.index = p, e.size = u, h(d.url, s, e, i.formData))) : r()) : r()), d.uploadOver && (i.onUploadSuccess && i.onUploadSuccess(n, s.responseText), setTimeout(function() {
                                        t.find("#fileupload_" + a + "_" + n.index).fadeOut()
                                    }, i.removeTimeout))
                                } else d.uploadOver && i.onUploadError && i.onUploadError(n, s.responseText);
                                d.uploadOver && (i.onUploadComplete && i.onUploadComplete(n, s.responseText), d.fileInput.val(""))
                            }
                        }, i.onUploadStart && i.onUploadStart(), i.formData.fileName = n.name, i.formData.lastModifiedDate = n.lastModifiedDate.getTime(), d.uploadAllowed = !0, h(this.url, s, e, i.formData))
                    },
                    init: function() {
                        this.fileInput.length > 0 && this.fileInput.change(function(e) {
                            d.funGetFiles(e)
                        }), t.find(".uploadify-button").on("click", function() {
                            t.find(".selectbtn").trigger("click")
                        }), i.onInit && i.onInit()
                    }
                }, d.init()
            });
            var p = {
                stop: function() {
                    d.uploadOver = !1, d.uploadAllowed = !1
                },
                upload: function(e) {
                    if ("*" === e)
                        for (var t = 0, a = d.fileFilter.length; a > t; t++) d.funUploadFile(d.fileFilter[t]);
                    else {
                        var s = o(e, d.fileFilter);
                        s && d.funUploadFile(s)
                    }
                },
                cancel: function(e) {
                    if ("*" === e)
                        for (var t = 0, a = d.fileFilter.length; a > t; t++) d.funDeleteFile(++t);
                    else d.funDeleteFile(e)
                },
                disable: function(t) {
                    var a = e(t ? "file_upload_" + t + "-button" : "body");
                    a.find(".uploadify-button").css("background-color", "#888").off("click")
                },
                ennable: function(t) {
                    var a = e(t ? "file_upload_" + t + "-button" : "body");
                    a.find(".uploadify-button").css("background-color", "#707070").on("click", function() {
                        a.find(".selectbtn").trigger("click")
                    })
                },
                destroy: function() {
                    d.container.html("")
                },
                settings: function(t, a) {
                    return 1 == arguments.length ? i[t] : void("formData" == t ? i.formData = e.extend(i.formData, a) : i[t] = a)
                },
                Huploadify: function() {
                    var e = arguments[0];
                    e in this && (Array.prototype.splice.call(arguments, 0, 1), this[e].apply(this[e], arguments))
                }
            };
            return p
        }
