...
class InitConfigBehavior extends Behavior{
    ...
    public function run(&$content){
        ...
        if(!$system_config){        	
            ...
            if(MODULE_NAME === 'Admin' || $controller_name[0] === 'Admin'){
            ...
            }elseif(MODULE_NAME === 'Manage' || $controller_name[0] === 'Manage'){
            	$system_config['MODULE_MARK'] = 'Manage';
            
            	//SESSION与COOKIE与前缀设置避免冲突
            	$system_config['SESSION_PREFIX'] = ENV_PRE.'Manage_'; //Session前缀
            	$system_config['COOKIE_PREFIX']  = ENV_PRE.'Manage_'; //Cookie前缀
            	if($controller_name[0] === 'Manage'){
            		$current_theme = 'Manage/'.$current_theme;
            	}
            	$system_config['TMPL_PARSE_STRING'] = C('TMPL_PARSE_STRING'); //先取出配置文件中定义的否则会被覆盖
            	$system_config['TMPL_PARSE_STRING']['__IMG__']  = __ROOT__.'/'.APP_PATH.MODULE_NAME.'/View/_Resource/img';
            	$system_config['TMPL_PARSE_STRING']['__CSS__']  = __ROOT__.'/'.APP_PATH.MODULE_NAME.'/View/_Resource/css';
            	...
            	//错误页面模板
            	$system_config['TMPL_ACTION_ERROR']   = APP_PATH.'Manage/View/_Think/error.html'; //错误跳转对应的模板文件
            	$system_config['TMPL_ACTION_SUCCESS'] = APP_PATH.'Manage/View/_Think/success.html'; //成功跳转对应的模板文件
            	$system_config['TMPL_EXCEPTION_FILE'] = APP_PATH.'Manage/View/_Think/exception.html'; //异常页面的模板文件
            }
